...
DECLARE @SrvSpecName             sysname = N'Audit_ToApplicationLog';
...
CREATE SERVER AUDIT Audit_ToAppLog
TO APPLICATION_LOG
WITH (ON_FAILURE = CONTINUE);
ALTER SERVER AUDIT Audit_ToAppLog WITH (STATE = ON);
...
BEGIN
    USE [AuditTestDB];
    PRINT 'Database AuditTestDB found. Continuing...';

    -- Check if table exists
    IF OBJECT_ID(N'dbo.TestTable', N'U') IS NOT NULL
    BEGIN
        PRINT 'Table dbo.TestTable found. Running test actions...';
        ...
        BEGIN TRY
            ...
            -- ALTER (ADD column)
            IF COL_LENGTH('dbo.TestTable', 'Email') IS NULL
            BEGIN
                ALTER TABLE dbo.TestTable ADD Email nvarchar(255) NULL;
                PRINT 'Column Email added.';
            END
            ELSE
                PRINT 'Column Email already exists. Skipping ADD.';

            -- ALTER (DROP column)
            IF COL_LENGTH('dbo.TestTable', 'Email') IS NOT NULL
            BEGIN
                ALTER TABLE dbo.TestTable DROP COLUMN Email;
                PRINT 'Column Email dropped.';
            END
            ELSE
                PRINT 'Column Email does not exist. Skipping DROP.';

            -- SELECT operations
            PRINT 'Running SELECT queries...';
            SELECT * FROM dbo.TestTable;
            SELECT TOP 5 UserName, CreatedAt FROM dbo.TestTable ORDER BY CreatedAt DESC;
            SELECT COUNT(*) AS TotalRows FROM dbo.TestTable;

        END TRY
        BEGIN CATCH
            PRINT 'Error occurred while running DDL/DML/SELECT test actions.';
            PRINT ERROR_MESSAGE();
        END CATCH
    END
    ELSE
    BEGIN
        PRINT 'Table dbo.TestTable does not exist in AuditTestDB. Skipping tests.';
    END
END
ELSE
BEGIN
    PRINT 'Database AuditTestDB does not exist. Skipping tests.';
END
GO
